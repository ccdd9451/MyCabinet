...
std::string constructBenchDataString(std::vector<unsigned int> points);
...
void ScenarioModule::init( const SteerLib::OptionDictionary & options, SteerLib::EngineInterface * engineInfo )
{
	...
	std::cout << " aiModuleName is " << _aiModuleName << " **\n";
			...
			else if ((*optionIter).first.substr(0, 10) == "circlePara") //1
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_circleParams.push_back(atof(tokens.at(i).c_str()));
					std::cout << "token " << i << "; " << _circleParams.at(i) << std::endl;
				}
			}
			else if ((*optionIter).first.substr(0, 8) == "hFinPara") //2
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_finHorizontalParams.push_back(atof(tokens.at(i).c_str()));
				}
			}
			else if ((*optionIter).first.substr(0, 8) == "vFinPara") //3
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_finVerticalParams.push_back(atof(tokens.at(i).c_str()));
				}
			}
			else if ((*optionIter).first.substr(0, 9) == "hDoorPara") //4
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_doorHorizontalParams.push_back(atof(tokens.at(i).c_str()));
				}

			}
			else if ((*optionIter).first.substr(0, 9) == "vDoorPara") //5
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_doorVerticalParams.push_back(atof(tokens.at(i).c_str()));
				}

			}
			else if ((*optionIter).first.substr(0, 7) == "boxPara") //6
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_boxParams.push_back(atof(tokens.at(i).c_str()));
				}

			}
			else if ((*optionIter).first.substr(0, 13) == "rotateBoxPara") //7
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_rotateBoxParams.push_back(atof(tokens.at(i).c_str()));
				}

			}
			else if ((*optionIter).first.substr(0, 10) == "rotBarPara") //8
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_rotBarParams.push_back(atof(tokens.at(i).c_str()));
				}
			}
			else if ((*optionIter).first.substr(0, 9) == "miscCPara") //9 Misc C
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_miscCParams.push_back(atof(tokens.at(i).c_str()));
				}
			}
			else if ((*optionIter).first.substr(0, 9) == "miscBPara") //10 Misc B
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_miscBParams.push_back(atof(tokens.at(i).c_str()));
				}
			}
			else if ((*optionIter).first.substr(0, 9) == "miscAPara") //11 Misc A
			{
				std::string params = (*optionIter).second;
				params.erase(std::remove(params.begin(), params.end(), '('), params.end());
				params.erase(std::remove(params.begin(), params.end(), ')'), params.end());
				...
				for (size_t i = 0; i < tokens.size(); i++)
				{
					_miscAParams.push_back(atof(tokens.at(i).c_str()));
				}
			}
			else if ((*optionIter).first.substr(0, 4) == "bar")
			{
				// _subspace = (*optionIter).second;
 ...
 */
			...
			else if ((*optionIter).first == "fixEnv") {
                _fixEnv = str2bool((*optionIter).second);
			}
			else if ((*optionIter).first == "fixAgent") {
                _fixAgent = str2bool((*optionIter).second);
			}
			else if ((*optionIter).first == "fixCrowd") {
                _fixCrowd = str2bool((*optionIter).second);
			}
			else if ((*optionIter).first == "readParam") {
                _readParam = str2bool((*optionIter).second);
			}
			else if ((*optionIter).first == "numReSim") {
                _numReSim = std::stod((*optionIter).second);
			}
			else if ((*optionIter).first == "Params") {
                std::istringstream iss((*optionIter).second);
...
*/
	...
	if ( _useBenchmark )
	{
		...
		// Layout parameters
		for (int l = 0; l < LAYOUTNUM; l++)
		{
			_benchmarkLogger->addDataField(std::to_string(l + 1) + "th_obstacle", DataType::Float);
		}

		// Crowd parameters
		for (int l = 0; l < CROWDNUM; l++)
		{
			_benchmarkLogger->addDataField(std::to_string(l + 1) + "th_region", DataType::Float);
		}

		// AI parameters
		for (int l = 0; l < AIPARAMNUM; l++)
		{
			_benchmarkLogger->addDataField(std::to_string(l + 1) + "th_ai", DataType::Float);
		}

		// total simulation metrics
		_benchmarkLogger->addDataField("collisionTimes", DataType::String);
		_benchmarkLogger->addDataField("collisionTotal", DataType::Float);
...
void ScenarioModule::loadSelfDefinedObstacles()
{
	// 1. Circle
	for (size_t i = 0; i < _circleParams.size(); i += 6) //x,z,r,x_b,z_b,r_b: 6
	{
		SteerLib::CircleObstacleInitialConditions * cic = new SteerLib::CircleObstacleInitialConditions();

		double x = _circleParams.at(i);
		double z = _circleParams.at(i + 1);
		double r = _circleParams.at(i + 2);
		double boundx = _circleParams.at(i + 3);
		double boundz = _circleParams.at(i + 4);
		double boundr = _circleParams.at(i + 5);

		//input 3 parameters
		double f = (double)rand() / RAND_MAX;
		if (_readParam)
		{
			x = _parameters[_indexParam + 0];
		}
		else
		{
			if (_currentScenario % _numReSim == 0)
			{
				if (_fixEnv) { f = 0.5; }
				x += (f - 0.5) * boundx; // inside bounding box
				_parameters[_indexParam + 0] = x;
			}
			else
			{
				x = _parameters[_indexParam + 0];
		...
		cic->position = Util::Point(x, 0.0, z);
		cic->radius = r;
		cic->height = 1.0f;
	...
	// 2. FinHorizon
	for (size_t i = 0; i < _finHorizontalParams.size(); i += 7) //x,z,l0(+/-),l1(+/-),a(+/-), xz_b, a_b: 7
	{
		SteerLib::OrientedBoxObstacleInitialConditions * ob0 = new SteerLib::OrientedBoxObstacleInitialConditions();
		SteerLib::OrientedBoxObstacleInitialConditions * ob1 = new SteerLib::OrientedBoxObstacleInitialConditions();

		double x = _finHorizontalParams.at(i + 0);
		double z = _finHorizontalParams.at(i + 1);
		double l0 = _finHorizontalParams.at(i + 2);
		double l1 = _finHorizontalParams.at(i + 3);
		double a = _finHorizontalParams.at(i + 4);
		double bound_xz = _finHorizontalParams.at(i + 5);
		double bound_a = _finHorizontalParams.at(i + 6);
		...
		ob0->lengthX = 1.0;
		ob0->lengthZ = abs(l0);
		ob0->position = Util::Point(x, 0.0, z + l0 / 2.0);
		ob0->thetaY = 0;
		ob0->height = 1.0f;
		...
		tmp.y = 0.0;
		tmp.z = 0.0;
		tmp = Util::rotateInXZPlane(tmp, radians(a));

		ob1->position = Util::Point(x + tmp.x, 0.0, z + l0 + tmp.z);
		ob1->thetaY = a;
		ob1->height = 1.0f;

		_indexParam += 2;

		_obstacles.push_back(ob0);
		_obstacles.push_back(ob1);
	}

	// 3. FinVertical
	for (size_t i = 0; i < _finVerticalParams.size(); i += 7) //x,z,l0(+/-),l1(+/-),a(+/-), xz_b, a_b: 7
	{
		SteerLib::OrientedBoxObstacleInitialConditions * ob0 = new SteerLib::OrientedBoxObstacleInitialConditions();
		SteerLib::OrientedBoxObstacleInitialConditions * ob1 = new SteerLib::OrientedBoxObstacleInitialConditions();

		double x = _finVerticalParams.at(i + 0);
		double z = _finVerticalParams.at(i + 1);
		double l0 = _finVerticalParams.at(i + 2);
		double l1 = _finVerticalParams.at(i + 3);
		double a = _finVerticalParams.at(i + 4);
		double bound_xz = _finVerticalParams.at(i + 5);
		double bound_a = _finVerticalParams.at(i + 6);

		//input 2 parameters
		double f = (double)rand() / RAND_MAX;
		if (_readParam)
		{
			z = _parameters[_indexParam + 0];
		}
		else
		{
			if (_currentScenario % _numReSim == 0)
		...
		ob0->lengthZ = 1.0;
		ob0->lengthX = abs(l0);
		ob0->position = Util::Point(x + l0 / 2.0, 0.0, z);
		ob0->thetaY = 0;
		ob0->height = 1.0f;
		...
		ob1->position = Util::Point(x + tmp.x + l0, 0.0, z + tmp.z);
		ob1->thetaY = a;
		ob1->height = 1.0f;
	...
	// 4. Horizontal Door
	for (size_t i = 0; i < _doorHorizontalParams.size(); i += 6) //x, z1,z2, zd, w, d_b: 6
	{
		double x = _doorHorizontalParams.at(i + 0);
		double z1 = _doorHorizontalParams.at(i + 1);
		double z2 = _doorHorizontalParams.at(i + 2);
		double zd = _doorHorizontalParams.at(i + 3);
		double w = _doorHorizontalParams.at(i + 4);
		double d_b = _doorHorizontalParams.at(i + 5);

		//input 1 parameters
		double f = (double)rand() / RAND_MAX;
		if (_readParam)
		{
			zd = _parameters[_indexParam];
		}
		else
		{
			if (_currentScenario % _numReSim == 0)
		...
		SteerLib::BoxObstacleInitialConditions * bic1 = new SteerLib::BoxObstacleInitialConditions(x - 0.5, x + 0.5, 0.0, 1.0, z1, zd - w / 2.0);
		SteerLib::BoxObstacleInitialConditions * bic2 = new SteerLib::BoxObstacleInitialConditions(x - 0.5, x + 0.5, 0.0, 1.0, zd + w / 2.0, z2);

		_obstacles.push_back(bic1);
		_obstacles.push_back(bic2);
	}

	// 5. Vertical Door
	for (size_t i = 0; i < _doorVerticalParams.size(); i += 6) //z, x1,x2, xd, w, d_b
	{
		double z = _doorVerticalParams.at(i + 0);
		double x1 = _doorVerticalParams.at(i + 1);
		double x2 = _doorVerticalParams.at(i + 2);
		double xd = _doorVerticalParams.at(i + 3);
		double w = _doorVerticalParams.at(i + 4);
		double d_b = _doorVerticalParams.at(i + 5);

		//input 1 parameters
		double f = (double)rand() / RAND_MAX;
		if (_readParam)
		{
			xd = _parameters[_indexParam];
		}
		else
		{
			if (_currentScenario % _numReSim == 0)
		...
		SteerLib::BoxObstacleInitialConditions * bic1 = new SteerLib::BoxObstacleInitialConditions(x1, xd - w / 2.0, 0.0, 1.0, z - 0.5, z + 0.5);
		SteerLib::BoxObstacleInitialConditions * bic2 = new SteerLib::BoxObstacleInitialConditions(xd + w / 2.0, x2, 0.0, 1.0, z - 0.5, z + 0.5);
	...
	// 6. Box
	for (size_t i = 0; i < _boxParams.size(); i += 6) //x,z,xl,zl,x_b,z_b: 6
	{
		double x = _boxParams.at(i + 0);
		double z = _boxParams.at(i + 1);
		double xl = _boxParams.at(i + 2);
		double zl = _boxParams.at(i + 3);
		double x_b = _boxParams.at(i + 4);
		double z_b = _boxParams.at(i + 5);
		...
		SteerLib::BoxObstacleInitialConditions * bic = new SteerLib::BoxObstacleInitialConditions(x - xl / 2.0, x + xl / 2.0, 0.0, 1.0, z - zl / 2.0, z + zl / 2.0);

		_obstacles.push_back(bic);
	}

	// 7. Roated Box
	for (size_t i = 0; i < _rotateBoxParams.size(); i += 8) //x,z,xl,zl,a, x_b,z_b,a_b: 8
	{
		double x = _rotateBoxParams.at(i + 0);
		double z = _rotateBoxParams.at(i + 1);
		double xl = _rotateBoxParams.at(i + 2);
		double zl = _rotateBoxParams.at(i + 3);
		double a = _rotateBoxParams.at(i + 4);
		double x_b = _rotateBoxParams.at(i + 5);
		double z_b = _rotateBoxParams.at(i + 6);
		double a_b = _rotateBoxParams.at(i + 7);
		...
		SteerLib::OrientedBoxObstacleInitialConditions * obo = new SteerLib::OrientedBoxObstacleInitialConditions();
		obo->lengthZ = zl;
		obo->lengthX = xl;
		obo->position = Util::Point(x, 0.0, z);
		obo->thetaY = a;
		obo->height = 1.0f;

		_obstacles.push_back(obo);
	}

	// 8. Roatated Bar
	for (size_t i = 0; i < _rotBarParams.size(); i += 5) //x,z,l,a, a_b: 5
	{
		SteerLib::OrientedBoxObstacleInitialConditions * ob = new SteerLib::OrientedBoxObstacleInitialConditions();

		double x = _rotBarParams.at(i + 0);
		double z = _rotBarParams.at(i + 1);
		double l = _rotBarParams.at(i + 2);
		double a = _rotBarParams.at(i + 3);
		double bound_a = _rotBarParams.at(i + 4);

		//input 1 parameters
		double f = (double)rand() / RAND_MAX;
		if (_readParam)
		{
			a = _parameters[_indexParam + 0];
		}
		else
		{
			if (_currentScenario % _numReSim == 0)
		...
		tmp.x = 0.0;
		tmp = Util::rotateInXZPlane(tmp, radians(a));
		ob->position = Util::Point(x + tmp.x, 0.0, z + tmp.z);

		_obstacles.push_back(ob);
	}

	// 9. Misc C
	for (size_t i = 0; i < _miscCParams.size(); i += 10) //x,z,l0(+/-),l1(+/-),l2,a1(+/-),a2, xz_b, a1_b, a2_b: 10
	{
		SteerLib::OrientedBoxObstacleInitialConditions * ob0 = new SteerLib::OrientedBoxObstacleInitialConditions();
		SteerLib::OrientedBoxObstacleInitialConditions * ob1 = new SteerLib::OrientedBoxObstacleInitialConditions();
		SteerLib::OrientedBoxObstacleInitialConditions * ob2 = new SteerLib::OrientedBoxObstacleInitialConditions();

		double x = _miscCParams.at(i + 0);
		double z = _miscCParams.at(i + 1);
		double l0 = _miscCParams.at(i + 2);
		double l1 = _miscCParams.at(i + 3);
		double l2 = _miscCParams.at(i + 4);
		double a1 = _miscCParams.at(i + 5);
		double a2 = _miscCParams.at(i + 6);
		double bound_xz = _miscCParams.at(i + 7);
		double bound_a1 = _miscCParams.at(i + 8);
		double bound_a2 = _miscCParams.at(i + 9);
		...
		tmp.y = 0.0;
		tmp.x = 0.0;
		tmp = Util::rotateInXZPlane(tmp, radians(a1));
		ob1->position = Util::Point(x + tmp.x + l0, 0.0, z + tmp.z);
		ob1->thetaY = a1;
		ob1->height = 1.0f;
		...
		tmp.y = 0.0;
		tmp.x = 0.0;
		tmp = Util::rotateInXZPlane(tmp, radians(a2));
		ob2->position = Util::Point(x + tmp.x + l0, 0.0, z + tmp.z);
		ob2->thetaY = a2;
		ob2->height = 1.0f;
	...
	// 11. Misc A
	for (size_t i = 0; i < _miscAParams.size(); i += 4) //x,z,l0(+/-), xz_b: 4
	{
		SteerLib::OrientedBoxObstacleInitialConditions * ob0 = new SteerLib::OrientedBoxObstacleInitialConditions();
		SteerLib::OrientedBoxObstacleInitialConditions * ob1 = new SteerLib::OrientedBoxObstacleInitialConditions();
		SteerLib::OrientedBoxObstacleInitialConditions * ob2 = new SteerLib::OrientedBoxObstacleInitialConditions();
		SteerLib::OrientedBoxObstacleInitialConditions * ob3 = new SteerLib::OrientedBoxObstacleInitialConditions();

		double x = _miscAParams.at(i + 0);
		double z = _miscAParams.at(i + 1);
		double l0 = _miscAParams.at(i + 2);
		double bound_xz = _miscAParams.at(i + 3);
		...
		ob1->lengthZ = 5.0;
		ob1->lengthX = 1.0;
		ob1->position = Util::Point(x + l0, 0.0, z + 2.5);
		ob1->thetaY = 0;
		ob1->height = 1.0f;

		ob2->lengthZ = 20.0;
		ob2->lengthX = 1.0;
		ob2->position = Util::Point(x + l0 / 2.0, 0.0, z - 10.0);
		ob2->thetaY = 0;
		ob2->height = 1.0f;

		ob3->lengthZ = 1.0;
		ob3->lengthX = 10.0;
		ob3->position = Util::Point(x + l0 / 2.0 + 5.0, 0.0, z - 20.0);
		ob3->thetaY = 0;
		ob3->height = 1.0f;
	...
	// 10. Misc B
	for (size_t i = 0; i < _miscBParams.size(); i += 6) //x,z,l0(+/-),a, xz_b, a_b: 6
	{
		SteerLib::OrientedBoxObstacleInitialConditions * ob0 = new SteerLib::OrientedBoxObstacleInitialConditions();
		SteerLib::OrientedBoxObstacleInitialConditions * ob1 = new SteerLib::OrientedBoxObstacleInitialConditions();
		SteerLib::OrientedBoxObstacleInitialConditions * ob2 = new SteerLib::OrientedBoxObstacleInitialConditions();

		double x = _miscBParams.at(i + 0);
		double z = _miscBParams.at(i + 1);
		double l0 = _miscBParams.at(i + 2);
		double a = _miscBParams.at(i + 3);
		double bound_xz = _miscBParams.at(i + 4);
		double bound_a = _miscBParams.at(i + 5);
		...
		ob2->lengthZ = 1.0;
		ob2->lengthX = 30.0;
		ob2->position = Util::Point(x, 0.0, z + l0);
		ob2->thetaY = 0;
		ob2->height = 1.0f;
		...
		ob1->position = Util::Point(x + tmp.x - 15.0, 0.0, z + l0 + tmp.z);
		ob1->thetaY = a;
		ob1->height = 1.0f;
...
void ScenarioModule::generateNextSubSpace()
{
	...
	if (_readParam)
	{
		...
		std::string line;
		std::ifstream inputfile("benchmarkInput.log");
		if (inputfile.is_open())
		{
			while (!inputfile.eof())
			{
				...
				if (line_num == _currentScenario + 3)
				{
					...
					unsigned int numP = 31 + 10 + 12;
					for (unsigned int f = 0; f < numP; f++)
					{
						pos = line.find(" ");
						token = line.substr(0, pos);
						line.erase(0, pos + 1);

						float param = std::stof(token);
						_parameters[f] = param;
					}
				}
	...
	for (unsigned int i=0; i < testCaseReader.getNumObstacles(); i++)
	{
		...
		else if (typeid(*testCaseReader.getObstacleInitialConditions(i)) == typeid(SteerLib::OrientedBoxObstacleInitialConditions))
		{
		...
		else if (typeid(*testCaseReader.getObstacleInitialConditions(i)) == typeid(SteerLib::OrientedWallObstacleInitialConditions))
		{
	...
	/***************************** Uniform Searching ************************************/
	SteerLib::AgentInitialConditions ic = testCaseReader.getAgentInitialConditions(0);
	int indexStart = gEngine->getSpatialDatabase()->getCellIndexFromLocation(ic.goals.at(0).targetLocation);
	gEngine->getSpatialDatabase()->uniformPlanning(indexStart);

	//Create the agents
	// Social Force
	float f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float acceleration = ACCELERATION * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float personal_space_threshold = PERSONAL_SPACE_THRESHOLD * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float agent_repulsion_importance = AGENT_REPULSION_IMPORTANCE * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float query_radius = QUERY_RADIUS * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float body_force = BODY_FORCE * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float agent_body_force = AGENT_BODY_FORCE * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float sliding_friction_force = SLIDING_FRICTION_FORCE * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float agent_b = AGENT_B * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float agent_a = AGENT_A * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float wall_b = WALL_B * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float wall_a = WALL_A * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float max_speed = MAX_SPEED * (f + 0.5);

	//ORCA
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float neighbor_distance = NEIGHBOR_DISTANCE * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float time_horizon = TIME_HORIZON * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float max_speed_orca = MAX_SPEED * (f + 0.5);
	f = (float)rand() / RAND_MAX;
	if (_fixAgent) { f = 0.5; }
	float time_horizon_obstacles = TIME_HORIZON_OBSTACLES * (f + 0.5);
	...
	if (_readParam)
	{
		...
		{
			//Social Force
			acceleration = _parameters[_indexParam + CROWDNUM + 0];
			personal_space_threshold = _parameters[_indexParam + CROWDNUM + 1];
			agent_repulsion_importance = _parameters[_indexParam + CROWDNUM + 2];
			query_radius = _parameters[_indexParam + CROWDNUM + 3];
			body_force = _parameters[_indexParam + CROWDNUM + 4];
			agent_body_force = _parameters[_indexParam + CROWDNUM + 5];
			sliding_friction_force = _parameters[_indexParam + CROWDNUM + 6];
			agent_b = _parameters[_indexParam + CROWDNUM + 7];
			agent_a = _parameters[_indexParam + CROWDNUM + 8];
			wall_b = _parameters[_indexParam + CROWDNUM + 9];
			wall_a = _parameters[_indexParam + CROWDNUM + 10];
			max_speed = _parameters[_indexParam + CROWDNUM + 11];
		}
		else
		{
			//ORCA
			neighbor_distance = _parameters[_indexParam + CROWDNUM + 0];
			time_horizon = _parameters[_indexParam + CROWDNUM + 1];
			max_speed_orca = _parameters[_indexParam + CROWDNUM + 2];
			time_horizon_obstacles = _parameters[_indexParam + CROWDNUM + 3];
		}
	}


	std::list<RegionInfo> regions = testCaseReader.getAgentRegion();
	unsigned int indexAgent = 0;
	for (std::list<RegionInfo>::iterator it = regions.begin(); it != regions.end(); ++it)
	{
		// std::cout << "working on agent " << i << std::endl;
		SteerLib::AgentInitialConditions ic = (*it).info;
		int numAgent = (*it).num;

		if (_readParam)
		{
			numAgent = (int)(_parameters[_indexParam]);
		}
		else
		{
			double f = (double)rand() / RAND_MAX;
			if (_fixCrowd) { f = 0.5; }
			numAgent = numAgent * 2.0 * f;

			_parameters[_indexParam] = numAgent;
		}

		_indexParam++;

		for (unsigned int k = 0; k < numAgent; k++)
		{
			...
			SocialForcesAgent* sfa = NULL;
			RVO2DAgent* orca = NULL;
            if (_aiModuleName == "sfAI")
                sfa = static_cast<SocialForcesAgent*>(agent);
            else if (_aiModuleName == "rvo2dAI")
                orca = static_cast<RVO2DAgent*>(agent);
            else
                std::cerr << "AI module not correct";
			if (sfa != NULL)
			{
				sfa->setFactors(acceleration, personal_space_threshold, agent_repulsion_importance, query_radius,
					body_force, agent_body_force, sliding_friction_force, agent_b, agent_a, wall_b, wall_a, max_speed);
			}
			if (orca != NULL)
			{
				orca->setFactors(neighbor_distance, time_horizon, max_speed_orca, time_horizon_obstacles);
			}

			std::vector<Util::Point> agentPath;
			ic.name = std::string("agent") + std::to_string(indexAgent);
	...
	if (!_readParam)
	{
		...
		{
			//Social Force
			_parameters[_indexParam + 0] = acceleration;
			_parameters[_indexParam + 1] = personal_space_threshold;
			_parameters[_indexParam + 2] = agent_repulsion_importance;
			_parameters[_indexParam + 3] = query_radius;
			_parameters[_indexParam + 4] = body_force;
			_parameters[_indexParam + 5] = agent_body_force;
			_parameters[_indexParam + 6] = sliding_friction_force;
			_parameters[_indexParam + 7] = agent_b;
			_parameters[_indexParam + 8] = agent_a;
			_parameters[_indexParam + 9] = wall_b;
			_parameters[_indexParam + 10] = wall_a;
			_parameters[_indexParam + 11] = max_speed;
			_indexParam += 12;
		}
		else
		{
			//ORCA
			_parameters[_indexParam + 0] = neighbor_distance;
			_parameters[_indexParam + 1] = time_horizon;
			_parameters[_indexParam + 2] = max_speed_orca;
			_parameters[_indexParam + 3] = time_horizon_obstacles;
			_indexParam += 4;
		}
	}
...
void ScenarioModule::generateNextSubSpaceWithoutObstacles()
{
	std::srand(std::time(0));
	_indexParam = 0;
	clearScenario();
...
ScenarioModule::postprocessFrame(float timeStamp, float dt, unsigned int frameNumber)
{
			...
			(int) frameNumber > _maxNumFrames || gEngine->getClock().getCurrentRealTime() > _scenarioTimeThreshold)
	{
		...
		if ( _useBenchmark )
		{
			...
			/*************Additional parameters for building scenario ****************/
			for (int i = 0; i < LAYOUTNUM + CROWDNUM + AIPARAMNUM; i++)
			{
				benchmarkLogObject.addLogData(_parameters[i]);
			}
...
#endif
			std::vector<unsigned int> collisionTimes;
			_benchmarkModule->getCollisionTimes(collisionTimes);

			DataItem collisions;
			collisions.string = constructBenchDataString(collisionTimes);
			benchmarkLogObject.addLogDataItem(collisions);
...
std::cout << "after clearing scenario \n";
		...
		if ( _currentScenario < _numScenarios)
		{
			...
			if ((_currentScenario % 5) == 0)
			{
				clearScenario();
			}
			else
			{
				clearScenarioKeepObstacles();
			}
...
void ScenarioModule::clearScenarioKeepObstacles()
{
	// cleaning up agents
